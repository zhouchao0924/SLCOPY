...
export default class RequestPage extends React.Component {
        ...
        render() {
            ...
            return ( < div className = "content-inner" >
                ...
                onChange = { this.handeleURLChange } > {
                    requestOptions.map((item, index) => {
                            ...
                            return ( < Select.Option key = { index }
                                value = { `${item.url}?${m}` } > { `${m.toLocaleUpperCase()}` } { item.url } < /Select.Option >)
                            })
                    } < /Select> < Button type = "primary"
                    style = {
                        { width: 100, marginLeft: 16 }
                    }
                    onClick = { this.handleRequest } > 发送 < /Button > </div > < div className = {
                        styles.params
                ...
                placeholder = "null" / > < div style = {
                    {
                        flex: 1,
                        marginLeft: 16
                    }
                } > { currntRequest.desc } < /div> < /div >
                < div className = { styles.result } > { result } < /div > </Card > < /Col> < /Row > < /div > )
            }
        }

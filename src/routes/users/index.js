...
function Users({location, dispatch, users, loading}) {
	...
	const userModalProps = {
		...
		onOk(data) {dispatch({type: `users/${modalType}`, payload: data})},
		onCancel() {dispatch({type: 'users/hideModal'})}
	}
	const userListProps = {
		dataSource: list,
		loading,
	...
	const userFilterProps = {
		...
		onSearch(fieldsValue) {
			...
			? dispatch(routerRedux.push({pathname: '/users',query: {field: fieldsValue.field,keyword: fieldsValue.keyword}}))
			: dispatch(routerRedux.push({pathname: '/users'}))
		},
		onAdd() {
			dispatch({type: 'users/showModal',payload: {modalType: 'create'}})
		},
		switchIsMotion() {
